...
void AccelStepper::setOutputPins(uint8_t mask)
{
    ...
    //     (mask & (1<<i)) ? (analogWrite(_pin[i],148)):(digitalWrite(_pin[i],(LOW ^ _pinInverted[i])));
    // }
   for (i = 0; i < numpins; i++) {
      if(mask == 0) digitalWrite(_pin[i], (mask & (1 << i)) ? (HIGH ^ _pinInverted[i]) : (LOW ^ _pinInverted[i]));
      else {
         digitalWrite(_pin[i], (mask & (1 << i)) ? (HIGH ^ _pinInverted[i]) : (LOW ^ _pinInverted[i]));
         delayMicroseconds(_minPulseWidth);
         digitalWrite(_pin[i], (mask & (1 << i)) ? (LOW ^ _pinInverted[i]) : (HIGH ^ _pinInverted[i]));
         delayMicroseconds(_minPulseWidth*9);
      }
   }
}
